...
export default function UploadZone({ selectedFile, onFileSelect, onFileRemove, errorMessage, onClearError }) {
    ...
    <div className="w-full">
      ...
      {!selectedFile ? (
        <div
          role="button"
          tabIndex={0}
          aria-label="Upload resume in PDF or Word document format, max 5MB"
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-soft flex items-center justify-between transition-all">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <p className="text-sm font-semibold text-slate-900 truncate max-w-[190px] sm:max-w-md">
                  {selectedFile.name}
                </p>
          ...
          <button
            ...
            className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500 flex-shrink-0 ml-2"
            title="Remove file"
          >
            <X className="w-5 h-5" />
          </button>
      ...
      {/* Error Banner with Try Again Action */}
      {activeError && (
        <div className="mt-4 p-4 rounded-xl bg-red-50 border border-red-200 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-sm text-red-700 animate-fadeIn">
          <div className="flex items-start gap-3">
            <AlertCircle className="w-5 h-5 flex-shrink-0 mt-0.5 text-red-500" />
            <div>
              <p className="font-semibold text-red-900">Upload Issue</p>
              <p className="text-xs mt-0.5 leading-relaxed text-red-700 max-w-xl">{activeError}</p>
            </div>
          </div>
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs font-semibold text-red-800 bg-red-100 hover:bg-red-200/80 border border-red-200 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-400 flex-shrink-0 self-end sm:self-center"
          >
